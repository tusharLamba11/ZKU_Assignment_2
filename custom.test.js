...
describe("Custom", function () {
  ...
  it("Insert pair of leaves to merkletree,Deposit from L1 and withdraw to L2.", async function () {
    const { merkleTreeWithHistory, tornadoPool, token, omniBridge } =
      await loadFixture(fixture);

    //Print the gas fees for inserting pair of leaves to the merkletree.
    const insertGas = await merkleTreeWithHistory.estimateGas.insert(
      toFixedHex(123),
      toFixedHex(456)
    );
    console.log(insertGas.toString());

    const aliceKeypair = new Keypair(); // contains both private and public keys

    // Alice deposits into tornado pool from L1
    ...
    //Sending tokens to omnibridge mock
    ...
    await omniBridge.execute([
     ...
     // Withdraw a part of Alice's funds from the pool to reciever.
